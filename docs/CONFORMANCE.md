...
## A `?:` whose arms build class temporaries destroys ones never constructed

One arm of a conditional runs; the other's temporaries are never built, and the
end of the full expression destroys them all the same.

```cpp
int take(T p) { return p.n; }
int r = b ? take(T(5)) : take(T(9));
```

Counted as live objects rather than constructor calls, so elision cannot move
the answer: clang ends with `live 0` and constructions balancing destructions,
cxx1 with `live -2` and four destructor calls where there was one construction
and one copy - two of them on frame slots nothing ever wrote, printing whatever
the stack held.

**The diagnosis, which is more useful than the symptom.** The temporaries
already carry a guard flag, set when the constructor returns, which the cleanup
pads read - see "A temporary during unwinding" in CLAUDE.md. The flags are
cleared in front of the full expression by `endFullExpression`, and a
*declaration* does not go through it: `declarationBody` flushes its temporaries
through `flushTemporaries`, which emits the destructors and never the clears.
So on the declaration path the guard is read from whatever the frame held, and
making the destructors ask it there is not enough on its own - measured, and it
turns two wrong destructions into two differently wrong ones.

What it needs is the clears emitted in front of the statements a declaration's
full expression contributed, which `flushTemporaries` cannot place because it
runs after them and does not know where they began. That is the piece to build,
and with it the class-typed `?:` lowering below becomes reachable.

**And the class-typed `?:` is refused for the same underlying reason.** Both
arms would have to build into one slot the expression owns - the lowering is
written and works for the arms themselves - but until an arm's temporaries are
that arm's alone, `b ? held : make(2)` destroys the result of a call that never
happened. `tests/cases/conditional-class-refused.cpp` pins the refusal.
