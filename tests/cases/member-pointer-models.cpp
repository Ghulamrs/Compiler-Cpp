...
struct Plain { int p; };
// A pointer to a virtual member function is refused by name in this
// compiler, so Poly's k is plain and its vfptr comes from another function.
struct Poly : Plain { virtual int v(); int k(); int q; };
int Poly::v() { return 0; }
